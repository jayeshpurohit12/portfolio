    ...
    >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        {projects.map((project: Project, index: number) => {
          return (
            <Card3D key={project.id} className="h-full" maxTilt={8}>
              <motion.article
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: index * 0.1 }}
                className="project-card flex flex-col group relative overflow-hidden bg-[#152031] border border-[#434656] hover:border-[#2e5bff] transition-all duration-300 shadow-xl h-full"
              >
